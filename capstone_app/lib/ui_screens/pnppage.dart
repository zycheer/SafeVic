...
class PnpPage extends StatefulWidget {
  @override
  _PnpPageState createState() => _PnpPageState();
}
...
class _PnpPageState extends State<PnpPage> with SingleTickerProviderStateMixin {
  ...
  // Debug flag to show all incidents or just PNP incidents
  bool _showDebugInfo = true;
  bool _showAllIncidents = false;

  // PNP incident categories
  static const List<String> pnpIncidentTypes = [
    'Theft/Robbery', 'Assault', 'Domestic Violence', 
            'Drug-related Crime', 'Vandalism', 'Fraud/Scam', 
            'Public Disturbance', 'Missing Person', 'Accident',
            'Traffic Accident', 'Burglary', 'Fight', 'Hit and Run',
  ...
  void initState() {
    ...
    // Subscribe to PNP topic
    FirebaseMessaging.instance.subscribeToTopic('pnp');

    // Listen for FCM messages
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      if (message.data['department'] == 'Police Department') {
        ...
        await _notificationsPlugin.show(
          0,
          'New Police Emergency',
          '$emergencyType reported by $userName',
          NotificationDetails(
            android: AndroidNotificationDetails(
              'pnp_channel',
              'PNP Emergencies',
              channelDescription: 'Notifications for police emergencies',
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('🚨 New Police Emergency: $emergencyType by $userName'),
...
Future<void> _sendPoliceResponderNotificationToUser(
  String incidentId, 
  ...
  try {
    ...
    final responderName = user?.displayName ?? user?.email ?? 'Police Responder';
    ...
    print('Sending police responder notification to Flask backend...');
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['success'] == true) {
        print('✓ Police responder notification sent successfully to user');
        
        // Show local success notification
        await _notificationsPlugin.show(
          1,
          'User Notified',
          'User has been notified that police are responding',
          NotificationDetails(
            android: AndroidNotificationDetails(
              'police_responder_channel',
              'Police Responder Actions',
              channelDescription: 'Notifications for police responder actions',
    ...
    print('Error sending police responder notification: $e');
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            SizedBox(width: 8),
            Expanded(
              child: Text('User notification failed, but police response was recorded'),
            ),
          ],
        ),
        backgroundColor: Colors.blue,
  ...
  Stream<QuerySnapshot> _getPnpIncidents() {
    print('=== QUERY DEBUG ===');
  ...
  // Check if an incident is PNP-related
  bool _isPnpIncident(Map<String, dynamic> data) {
    ...
    for (String pnpType in pnpIncidentTypes) {
      if (category == pnpType.toLowerCase() || 
          incidentCategory == pnpType.toLowerCase() ||
          emergencyType == pnpType.toLowerCase() ||
          type == pnpType.toLowerCase()) {
        isPnp = true;
        break;
      }
    }
    ...
    if (!isPnp) {
      isPnp = _containsPnpKeywords(allText);
    }
    
    if (_showDebugInfo && isPnp) {
      print('✓ PNP Incident found: category="$category", emergencyType="$emergencyType", type="$type"');
    }
    
    return isPnp;
  }

  // Comprehensive keyword checking for PNP incidents
  bool _containsPnpKeywords(String text) {
    if (text.isEmpty) return false;
    
    final pnpKeywords = [
      'theft', 'robbery', 'steal', 'stolen', 'burglary', 'break-in', 'mugging',
      'assault', 'attack', 'battery', 'physical violence', 'fight', 'brawl',
      'domestic violence', 'spousal abuse', 'family violence', 'partner violence',
      'drug', 'narcotics', 'shabu', 'marijuana', 'cocaine', 'drug trafficking', 'drug possession',
      'vandalism', 'graffiti', 'property damage', 'destruction', 'defacing',
      'fraud', 'scam', 'swindling', 'deception', 'identity theft', 'phishing',
      'public disturbance', 'disturbance', 'nuisance', 'unruly behavior', 'disorderly conduct',
      'missing person', 'missing', 'disappearance', 'lost person',
      'crime', 'criminal', 'illegal', 'law violation', 'police', 'pnp'
    ];
    
    return pnpKeywords.any((keyword) => text.contains(keyword));
  }

  // Better incident categorization for PNP
  String _categorizeIncident(Map<String, dynamic> data) {
    ...
    for (String field in fieldsToCheck) {
      if (pnpIncidentTypes.contains(field)) {
        return field;
      }
    }
    ...
    if (_containsKeywords(allText, ['theft', 'robbery', 'steal', 'stolen', 'burglary', 'mugging'])) {
      return 'Theft/Robbery';
    }
    if (_containsKeywords(allText, ['assault', 'attack', 'battery', 'physical violence', 'fight', 'brawl'])) {
      return 'Assault';  
    }
    if (_containsKeywords(allText, ['domestic violence', 'spousal abuse', 'family violence', 'partner violence'])) {
      return 'Domestic Violence';
    }
    if (_containsKeywords(allText, ['drug', 'narcotics', 'shabu', 'marijuana', 'cocaine', 'drug trafficking'])) {
      return 'Drug-related Crime';
    }
    if (_containsKeywords(allText, ['vandalism', 'graffiti', 'property damage', 'destruction'])) {
      return 'Vandalism';
    }
    if (_containsKeywords(allText, ['fraud', 'scam', 'swindling', 'deception', 'identity theft'])) {
      return 'Fraud/Scam';
    }
    if (_containsKeywords(allText, ['public disturbance', 'disturbance', 'nuisance', 'unruly behavior'])) {
      return 'Public Disturbance';
    }
    if (_containsKeywords(allText, ['missing person', 'missing', 'disappearance', 'lost person'])) {
      return 'Missing Person';
    }
    if (_containsKeywords(allText, ['crime', 'criminal', 'illegal', 'law violation'])) {
      return 'Other Crime';
    }
  ...
  try {
    ...
    await FirebaseFirestore.instance.collection('incidents').doc(incidentId).update({
      'status': 'active',
      'assignedTo': 'PNP Team',
      'assignedResponder': user.displayName ?? user.email ?? 'PNP Responder',
      'responseTime': FieldValue.serverTimestamp(),
      'lastUpdated': FieldValue.serverTimestamp(),
      'updatedBy': user.email ?? 'PNP',
      'isActive': true,
      'incidentCategory': incidentCategory,
      'responseTeam': _getResponseTeamForIncident(incidentCategory),
    });

    // Send notification to user that police are responding
    await _sendPoliceResponderNotificationToUser(
      incidentId, 
      incidentData, 
      'responded',
      incidentCategory
  ...
  String _getResponseTeamForIncident(String incidentCategory) {
    switch (incidentCategory) {
      case 'Theft/Robbery':
      case 'Assault':
        return 'Patrol Response Team';
      case 'Domestic Violence':
        return 'Family Protection Unit';
      case 'Drug-related Crime':
        return 'Anti-Drug Operations Team';
      case 'Vandalism':
      case 'Public Disturbance':
        return 'Public Safety Team';
      case 'Fraud/Scam':
        return 'Anti-Fraud Unit';
      case 'Missing Person':
        return 'Search and Investigation Team';
      default:
        return 'Police Response Team';
    }
  }
...
String _getResponseMessage(String incidentCategory) {
  switch (incidentCategory) {
    case 'Theft/Robbery':
      return 'Police patrol units are dispatched to secure the area and investigate!';
    case 'Assault':
      return 'Emergency response team is en route with medical assistance coordination!';
    case 'Domestic Violence':
      return 'Family protection unit is responding with crisis intervention support!';
    case 'Drug-related Crime':
      return 'Anti-drug operations team is deployed to handle the situation!';
    case 'Vandalism':
      return 'Public safety team is investigating and securing the affected area!';
    case 'Fraud/Scam':
      return 'Anti-fraud unit has been activated to investigate the incident!';
    case 'Public Disturbance':
      return 'Public safety team is dispatched to restore order and ensure safety!';
    case 'Missing Person':
      return 'Search and investigation team is activated to locate the individual!';
    default:
      return 'Police units are responding immediately to handle the situation!';
  }
}
...
void _showSuccessDialog(String title, String message) {
  ...
  Widget _buildIncidentCard(DocumentSnapshot doc) {
    ...
    final emergencyType = data['emergencyType'] ?? data['incidents'] ?? data['type'] ?? 'Crime Incident';
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Is PNP: ${_isPnpIncident(data)} | Matches Filter: ${_matchesStatusFilter(data)}',
                          style: TextStyle(
                            fontSize: isSmallScreen ? 8 : 10, 
                            color: Colors.grey[600]
                          ),
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          SizedBox(width: 8),
          _buildFilterChip('active', 'Active', Icons.security),
          SizedBox(width: 8),
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        for (var doc in allDocs) {
          final data = doc.data() as Map<String, dynamic>;
          if (_showAllIncidents || _isPnpIncident(data)) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text(
                    'No PNP incidents match your current filter.',
                    style: TextStyle(
                      fontSize: bodyFontSize,
        ...
        final filteredDocs = allDocs.where((doc) {
          ...
          // Filter by PNP incidents or show all if debug enabled
          bool isPnpRelated = _showAllIncidents || _isPnpIncident(data);
          if (!isPnpRelated) return false;
            ...
            child: Column(
              ...
              children: [
                ...
                SwitchListTile(
                  ...
                  subtitle: Text(
                    'Display all incidents, not just PNP-related ones',
                    style: TextStyle(fontSize: smallFontSize),
                  ),
          ...
          Container(
            ...
            child: ElevatedButton.icon(
              onPressed: () async {
                bool confirmed = await _showConfirmationDialog(
                  'Logout',
                  'Are you sure you want to logout from the PNP Emergency Response System?',
                );
                if (confirmed) {
                  await _logout();
                }
